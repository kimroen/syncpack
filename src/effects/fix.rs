          ...
          if instance_obj.is_fixable() || cannot_autofix {
            ...
            if instance_obj.is_fixable() {
              if instance_obj.is_banned() {
                ...
                let location = instance::get_location(&ctx, instance_obj).dimmed();
                let state_link = instance::get_state_link_in_parens(&ctx, instance_obj, &group.variant);
                info!("  {location} {state_link}");
                instance_obj.remove()
              } else {
                let name = &instance_obj.descriptor.name;
                let expected = instance::get_expected(instance_obj).dimmed();
                let location = instance::get_location(&ctx, instance_obj).dimmed();
              ...
              is_invalid = true;
              let name = &instance_obj.descriptor.name;
              let actual = instance::get_actual(instance_obj);
              let location = instance::get_location(&ctx, instance_obj).dimmed();
              let state_link = instance::get_state_link_in_parens(&ctx, instance_obj, &group.variant);
